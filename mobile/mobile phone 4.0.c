...
void startup(struct master *file)
{
	...
	file->database = fopen("contacts.txt", "rb"); //opens contanct database
	if(file->database)
	{
...
void close(struct master *file)
{
	...
	file->database = fopen("contacts.txt", "wb");

	for (i = 0; i < file->existing; ++i)
	{
		fwrite(file->contact[i], sizeof(struct contacts), 1, file->database);
	}
...
void load (struct master *file)
{
	...
	file->existing = cCont(file->database);	//reads existing amount of contacts by reading database lines
	printf("%d\n", file->existing);
	system("pause");
	...
	for (i = 0; i < file->existing; ++i)												//reads them
	{
		...
		file->contact[i]->name = malloc(sizeof(char)*255);
		file->contact[i]->phone = malloc(sizeof(char)*255);
		fread(file->contact[i], sizeof(struct contacts), 1, file->database);
		printf("%s %s\n",file->contact[i]->name, file->contact[i]->phone );
	}
}
...
int  cCont		(FILE *stream)
{
	...
	fseek(stream, 0, SEEK_END);						//Set cursor to end of file
	size = ftell(stream)/sizeof(struct contacts);	//Calculates how many contacts exist
	fseek(stream, 0, SEEK_SET);						//Reset cursor start of file
